...
**Produced:** 2026-04-29T14:33:14.574267+00:00
...
| 2.1 | cloud_sql | Cloud SQL connection via Auth Proxy | ✅ PASS | PostgreSQL: PostgreSQL 15.17 on x86_64-pc-linux-gnu, compiled by Debian ... | 493ms |
| 2.2 | cloud_sql | charts (must have ≥1 chart) | ✅ PASS | 1 rows | 114ms |
| 2.3 | cloud_sql | msr_signals (L2.5 — L2.5 MSR signals — pipeline tool: msr_sql) | ✅ PASS | 499 rows | 172ms |
| 2.4a | cloud_sql | rag_chunks (L1+L2.5 — non-stale RAG chunks — pipeline tool: vector_search) | ✅ PASS | 941 rows | 67ms |
| 2.4b | cloud_sql | rag_embeddings (vector embeddings — pipeline tool: vector_search) | ✅ PASS | 941 rows | 52ms |
| 2.5a | cloud_sql | l25_cgm_nodes (L2.5 CGM — CGM graph nodes — tool: cgm_graph_walk) | ✅ PASS | 234 rows | 47ms |
| 2.5b | cloud_sql | l25_cgm_edges_valid (L2.5 CGM — CGM graph edges (valid) — tool: cgm_graph_walk) | ✅ PASS | 21 rows | 47ms |
| 2.5c | cloud_sql | l25_cgm_edges_total (L2.5 CGM — CGM graph edges (all statuses)) | ✅ PASS | 127 rows | 52ms |
| 2.6a | cloud_sql | ucn_sections (L2.5 UCN — row count) | ❌ FAIL | relation "ucn_sections" does not exist LINE 1: SELECT COUNT(*) FROM ucn_sections                              ^ | 91ms |
| 2.6b | cloud_sql | cdlm_links (L2.5 CDLM — row count) | ❌ FAIL | current transaction is aborted, commands ignored until end of transaction block | 55ms |
| 2.6c | cloud_sql | cgm_nodes (L2.5 CGM nodes — row count) | ❌ FAIL | current transaction is aborted, commands ignored until end of transaction block | 52ms |
| 2.6d | cloud_sql | cgm_edges (L2.5 CGM edges — row count) | ❌ FAIL | current transaction is aborted, commands ignored until end of transaction block | 50ms |
| 2.6e | cloud_sql | rm_resonances (L2.5 RM — row count) | ❌ FAIL | current transaction is aborted, commands ignored until end of transaction block | 48ms |
...
| 2.6g | cloud_sql | eclipses (L1 eclipses — row count) | ❌ FAIL | current transaction is aborted, commands ignored until end of transaction block | 50ms |
| 2.6h | cloud_sql | retrogrades (L1 retrogrades — row count) | ❌ FAIL | current transaction is aborted, commands ignored until end of transaction block | 50ms |
| 2.6i | cloud_sql | life_events (L1 life events — row count) | ❌ FAIL | current transaction is aborted, commands ignored until end of transaction block | 49ms |
| 2.6j | cloud_sql | sade_sati (L1 Sade Sati cycles — row count) | ❌ FAIL | current transaction is aborted, commands ignored until end of transaction block | 112ms |
| 2.7a | cloud_sql | audit_events (pipeline audit log) | ❌ FAIL | current transaction is aborted, commands ignored until end of transaction block | 51ms |
| 2.7b | cloud_sql | query_trace_steps (pipeline trace steps) | ❌ FAIL | current transaction is aborted, commands ignored until end of transaction block | 48ms |
| 2.8 | cloud_sql | conversations (persisted conversations) | ❌ FAIL | current transaction is aborted, commands ignored until end of transaction block | 50ms |
...
| 3.1 | gcs | Chat attachments bucket (upload/download for chat files) [madhav-astrology-chat-attachments] | ✅ PASS | Accessible (sampled 1 object) | 1666ms |
| 3.2 | gcs | Chart documents bucket (reports PDF storage) [madhav-astrology-chart-documents] | ✅ PASS | Accessible (sampled 1 object) | 224ms |
| 3.3 | gcs | Sources bucket (pipeline reads VALIDATED_ASSET_REGISTRY + corpus files) [madhav-marsys-sources] | ✅ PASS | Accessible (sampled 1 object) | 689ms |
| 3.4 | gcs | Build artifacts bucket (pipeline writes build manifests) [madhav-marsys-build-artifacts] | ✅ PASS | Accessible (sampled 1 object) | 678ms |
...
| 6.1 | tool | msr_sql — Cloud SQL: msr_signals table | ✅ PASS | 499 signals. Top domains: {'career': 208, 'spirit': 94, 'wealth': 64, 'relationships': 41, 'health': 32} | 1068ms |
...
| 6.8 | tool | cgm_graph_walk — l25_cgm_nodes + l25_cgm_edges (L2.5 CGM) | ✅ PASS | nodes=234  edges_valid=21  edges_total=127  types={'SUPPORTS(orphan)': 101, 'DISPOSITED_BY(valid)': 8, 'NAKSHATRA_LORD_I | 1044ms |
...
| 6.10 | tool | vector_search — Vertex AI embed + Cloud SQL: rag_embeddings ⊕ rag_chunks | ✅ PASS | chunks=941  embeddings=941  coverage=100.0%  doc_types={'msr_signal': 499, 'cgm_node': 234, 'l1_fact': 102, 'cdlm_cell': | 1600ms |
| 7.1 | anthropic | Anthropic API key format | ✅ PASS | Key present: sk-ant-...sFZD2gAA | — |
| 7.2 | anthropic | Anthropic API reachable (models endpoint) | ✅ PASS | HTTP 200 — sample models: ['claude-opus-4-7', 'claude-sonnet-4-6', 'claude-opus-4-6'] | 497ms |
