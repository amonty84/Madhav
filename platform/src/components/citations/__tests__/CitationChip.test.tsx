...
describe('CitationChip', () => {
  it('renders with correct aria-label for signal type', () => {
    ...
    const btn = screen.getByRole('button', { name: /citation: signal s0042/i })
    expect(btn).toBeInTheDocument()
  })

  it('renders with correct aria-label for asset type', () => {
    render(<CitationChip type="asset" id="CGM" />)
    expect(screen.getByRole('button', { name: /citation: asset cgm/i })).toBeInTheDocument()
  })

  it('renders with correct aria-label for chunk type', () => {
    render(<CitationChip type="chunk" id="rag-abc" />)
    expect(screen.getByRole('button', { name: /citation: chunk rag-abc/i })).toBeInTheDocument()
  })

  it('opens CitationPreview on click', () => {
    render(<CitationChip type="signal" id="S0001" />)
    const btn = screen.getByRole('button', { name: /citation: signal s0001/i })
    fireEvent.click(btn)
  ...
  it('closes CitationPreview when panel calls onClose', () => {
    render(<CitationChip type="signal" id="S0001" />)
    fireEvent.click(screen.getByRole('button', { name: /citation: signal s0001/i }))
    expect(screen.getByRole('dialog')).toBeInTheDocument()
  ...
  it('sets aria-expanded=true when preview is open', () => {
    render(<CitationChip type="asset" id="CGM" />)
    const btn = screen.getByRole('button', { name: /citation: asset cgm/i })
  ...
  it('is keyboard accessible via Enter key', () => {
    ...
    const btn = screen.getByRole('button', { name: /citation: chunk c-99/i })
    btn.focus()
