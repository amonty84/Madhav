...
- **Adjudication #2113/#2180** — RULED cycle 155 (waves 0-3, 15 assets, confirmed); wave 0
  ...
  `ga_positions` (migration 868, PR #2205) and `ga_ayurdaya` (migration 869, PR #2208, cycle
  160) — 2/7. **Still open**: `natural_key_partition` for the other 5 `chart_facts` co-writers
  (`ga_nakshatra`, `ga_panchanga`, `ga_sade_sati`, `ga_sensitive`, `ga_sensitive_degree` —
  `ga_sensitive` in particular needs careful per-category verification, found to own ~25
  categories, more than initially assumed) and `ga_positions`' own `output_digest_spec`
  (fix 2) — both needed before wave 1 can genuinely dispatch.
...
2/7 shipped — `ga_positions` PR #2205, `ga_ayurdaya` PR #2208; 5 co-writers' partitions +
