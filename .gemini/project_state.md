...
_Last updated: 2026-04-28 at Madhav_PHASE11A_CUTOVER_STAGE1 (Phase 11A Pipeline Cutover Stage 1 — COMPLETE). NEW_QUERY_PIPELINE_ENABLED + AUDIT_ENABLED defaults flipped to true in feature_flags.ts. New pipeline is now default behavior; legacy is opt-out (MARSYS_FLAG_NEW_QUERY_PIPELINE_ENABLED=false). Smoke script: npm run cutover:stage1-smoke. red_team_counter: 2 (governance aside). Next session: Madhav_M2A_Exec_15 (B.9)._
